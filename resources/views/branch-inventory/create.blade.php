...
@if ($errors->any())
    <div class="alert alert-danger py-2 px-3 mb-2" style="font-size:.8rem">
        <i class="bi bi-exclamation-triangle me-1"></i>
        @foreach ($errors->all() as $error)
            {{ $error }}@if (! $loop->last) &middot; @endif
        @endforeach
    </div>
@endif
@if (session('error'))
...
<div class="header-card">
    <div class="header-grid">
        ...
        {{-- DR Number (auto-generated by default; can switch to manual for legacy DRs) --}}
        <div>
            <label class="field-label">DR # <span class="field-req" id="drReqMark" style="display:none">*</span></label>
            ...
            <input type="hidden" name="dr_mode" id="drModeFlag" value="auto">
            <div class="d-flex align-items-center justify-content-between" style="margin-top:.15rem;gap:.4rem">
                <span id="drHint" style="font-size:.62rem;color:var(--text-muted)">Auto-generated on save</span>
                <span id="btnManualDr" role="button" style="font-size:.62rem;color:var(--accent);font-weight:700;cursor:pointer;white-space:nowrap;text-decoration:underline">
                    <i class="bi bi-pencil"></i> Manual DR#
                </span>
            </div>
        </div>
        {{-- Date --}}
...
var drManualMode = false;
var autoNextDr    = '{{ $nextDrNumber }}';

/** Toggle the DR # field between auto-generated (locked) and manual entry
 *  (for encoding legacy/previous DRs). The server always re-validates
 *  manual numbers for uniqueness and skips them automatically in future
 *  auto-generated numbers once saved. */
function setDrMode(manual) {
    drManualMode = manual;
    var input = document.getElementById('drNumberDisplay');
    var flag  = document.getElementById('drModeFlag');
    var btn   = document.getElementById('btnManualDr');
    var hint  = document.getElementById('drHint');
    var req   = document.getElementById('drReqMark');

    if (manual) {
        input.removeAttribute('disabled');
        input.removeAttribute('readonly');
        input.setAttribute('name', 'dr_number');
        input.value = '';
        input.style.background = '';
        input.style.color = '';
        input.placeholder = 'Type existing / legacy DR #';
        input.focus();
        flag.value = 'manual';
        btn.innerHTML = '<i class="bi bi-arrow-counterclockwise"></i> Use Auto DR#';
        hint.textContent = 'Manual entry — must not duplicate an existing DR #';
        req.style.display = '';
    } else {
        input.setAttribute('disabled', 'disabled');
        input.setAttribute('readonly', 'readonly');
        input.removeAttribute('name');
        input.value = autoNextDr;
        input.style.background = 'var(--bg-page)';
        input.style.color = 'var(--accent)';
        flag.value = 'auto';
        btn.innerHTML = '<i class="bi bi-pencil"></i> Manual DR#';
        hint.textContent = 'Auto-generated on save';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    @if (old('dr_mode') === 'manual')
        setDrMode(true);
        document.getElementById('drNumberDisplay').value = '{{ old('dr_number') }}';
    @endif
    ...
    function validateDeliveryFormForReview() {
        ...
        if (drManualMode && !String(document.getElementById('drNumberDisplay').value).trim()) {
            showToast('Enter a DR number, or switch back to auto-generated.', 'danger');
            return false;
        }
    ...
    document.getElementById('btnManualDr').addEventListener('click', function() { setDrMode(!drManualMode); });
});
